...
exports.rawTimeClock = rawTimeClock;
exports.timeClock = timeClock;
const parse_csv_js_1 = require("../utils/parse-csv.js");
function rawTimeClock(caller, command, userId, projectId, serviceId) {
...
async function timeClock(caller, command, userId, projectId, serviceId) {
    const content = await rawTimeClock(caller, command, userId, projectId, serviceId);
    if (command !== "status") {
        return (await (0, parse_csv_js_1.parseCsv)("result\n" + content))[0];
    }
    else {
        return (await (0, parse_csv_js_1.parseCsv)("result;state;startOfTimeClock;startOfPause\n" + content, (row) => {
            const result = {
                result: row.result,
                state: row.state,
            };
