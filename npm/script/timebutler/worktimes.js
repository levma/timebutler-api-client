...
exports.rawWorktimes = rawWorktimes;
exports.worktimes = worktimes;
const create_start_and_end_dates_from_api_data_js_1 = require("../utils/create-start-and-end-dates-from-api-data.js");
 ...
 */
function rawWorktimes(caller, year, month) {
    const params = year
        ? { year: year.toString() }
        : undefined;
 ...
 */
async function worktimes(caller, timeZone = "Europe/Berlin", year, month) {
    return (0, parse_csv_js_1.parseCsv)(await rawWorktimes(caller, year, month), (row) => {
        const workingTimeInSeconds = parseInt(row["Working time in seconds"], 10);
        const pauseInSeconds = parseInt(row["Pause in seconds"], 10);
        const { startTime, endTime } = (0, create_start_and_end_dates_from_api_data_js_1.createStartAndEndDatesFromApiData)(row["Date (dd/mm/yyyy)"], row["Start time (hh:mm)"], row["End time (hh:mm)"], timeZone);
        const result = {
