 ...
 */
export function rawTimeImportByEvents(caller, csvdatafile, useridentification) {
 ...
 */
export async function timeImportByEvents(caller, csvdatafile, useridentification) {
    const str = await rawTimeImportByEvents(caller, csvdatafile, useridentification);
    if (str === "OK") {
        return { success: true };
    }
    const errors = await parseCsv("line;error\n" + str);
