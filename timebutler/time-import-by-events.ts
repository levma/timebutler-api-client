import type {
  TimeImportByEventsLineError,
  TimeImportByEventsResponse,
} from "../models/time-import-by-events-response.ts";
import type { UserIdentificationParamter } from "../models/user-identification-parameter.ts";
import { parseCsv } from "../utils/parse-csv.ts";
import type { TimebutlerApiCaller, TimebutlerApiParams } from "./api-caller.ts";
 ...
 */
export function rawTimeImportByEvents(
  caller: TimebutlerApiCaller,
  csvdatafile: File,
  useridentification?: UserIdentificationParamter,
): Promise<string> {
  const params: TimebutlerApiParams | undefined = useridentification
    ? { useridentification }
    : undefined;
 ...
 */
export async function timeImportByEvents(
  caller: TimebutlerApiCaller,
  csvdatafile: File,
  useridentification?: UserIdentificationParamter,
): Promise<TimeImportByEventsResponse> {
  const str = await rawTimeImportByEvents(
    caller,
    csvdatafile,
    useridentification,
  );
  ...
  const errors = await parseCsv<TimeImportByEventsLineError>(
    "line;error\n" + str,
  );
